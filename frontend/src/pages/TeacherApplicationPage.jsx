...
import {
  FiSearch, FiFileText, FiAlertCircle,
  FiClock, FiCheckCircle, FiXCircle, FiChevronRight, FiEye
} from 'react-icons/fi'
...
export default function TeacherApplications() {
  ...
  // ── Fetch all applications the teacher can see ────────────────
  useEffect(() => {
  ...
  const stats = [
    ...
    { label: 'Under Review', value: applications.filter(a => a.status === 'UNDER_REVIEW').length, icon: <FiEye         className="w-4 h-4" />, color: 'text-blue-600 dark:text-blue-400',    bg: 'bg-blue-50 dark:bg-blue-900/20'     },
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-light-muted dark:text-dark-muted text-sm mt-1">
            Review applications and add your recommendation before the admin decides.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search by child, parent, school or reference..."
              value={search}
        ...
        {filtered.length === 0 ? (
          ...
          <div className="bg-light-surface dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-2xl overflow-hidden">
            ...
            <div className="hidden md:grid grid-cols-12 gap-4 px-5 py-3 border-b border-light-border dark:border-dark-border bg-light-bg dark:bg-dark-bg">
              <p className="col-span-3 text-xs font-semibold text-light-muted dark:text-dark-muted uppercase tracking-wider">Child / Parent</p>
            ...
            <div className="divide-y divide-light-border dark:divide-dark-border">
              {filtered.map(app => {
                ...
                // Highlight PENDING rows so teacher knows which ones need attention
                const needsReview = app.status === 'PENDING'

                return (
                  <div
                    key={app.id}
                    onClick={() => navigate(`/dashboard/teacher/applications/${app.id}`)}
                    className={`
                      grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-4
                      px-5 py-4 cursor-pointer transition-colors group
                      ${needsReview
                  ...
                  >
                    ...
                    <div className="md:col-span-3">
                      <div className="flex items-center gap-2">
                        {/* Dot indicator for pending */}
                        {needsReview && (
                          <div className="w-1.5 h-1.5 rounded-full bg-yellow-500 flex-shrink-0" />
                        )}
                        <p className="text-sm font-semibold text-light-text dark:text-dark-text">{app.childName}</p>
                      </div>
                      <p className="text-xs text-light-muted dark:text-dark-muted mt-0.5 ml-3.5">
            ...
            {/* Footer */}
            <div className="px-5 py-3 border-t border-light-border dark:border-dark-border bg-light-bg dark:bg-dark-bg flex items-center justify-between">
              <p className="text-xs text-light-muted dark:text-dark-muted">
                Showing {filtered.length} of {applications.length} applications
              </p>
              {applications.filter(a => a.status === 'PENDING').length > 0 && (
                <p className="text-xs text-yellow-600 dark:text-yellow-400 font-medium">
                  {applications.filter(a => a.status === 'PENDING').length} awaiting your review
                </p>
              )}
            </div>
          </div>
